  ...
  type StatusType = 'succeeded' | 'processing' | 'requires_payment_method' | 'default' | 'canceled';

  const STATUS_CONTENT_MAP: Record<StatusType, { text: string; iconColor: string; icon: React.ReactNode }> = {
    succeeded: {
  ...
  export default function Complete() {
    ...
    const validStatuses: StatusType[] = ['succeeded', 'processing', 'requires_payment_method', 'default', 'canceled'];
    ...
    React.useEffect(() => {
      ...
      stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
        ...
        if (validStatuses.includes(paymentIntent.status as StatusType)) {
